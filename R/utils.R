...
apply_entity_metadata_defaults <- function(metadata, verbose = FALSE) {
  if (!is.na(metadata$name) && is.na(metadata$display_name)) {
    ...
    if (verbose) cat(glue("Added default display_name, '{metadata$display_name}', for entity"),"\n")
  }
  if (!is.na(metadata$display_name) && is.na(metadata$display_name_plural)) {
    ...
    if (verbose) cat(glue("Added default display_name_plural, '{metadata$display_name_plural}', for entity"),"\n")
  }
  metadata
}
